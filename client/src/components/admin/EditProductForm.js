...
import { addProduct, editProduct, loadDressTypes, setCurrentProduct } from "../../actions/appActions";
import { useParams } from 'react-router-dom';
import Loader from "./spinner";
import { API } from "../../constants/constants";
const EditProductForm = ({ addingProduct, loadDressTypes, dressTypeOptions, setCurrentProduct, currentProduct, editProduct }) => {
    ...
    useEffect(() => {
        if (currentProduct != null) {
            ...
            images = images.map(i => `${API}/uploads/${i}`)
            setPlaceHolder(images);
            setInStock(currentProduct.inStock);
            setdata(currentProduct);
            let defaultDressSize = currentProduct.dressSize.map(d => {
                for (const dressSize of OPTIONS.dressSizeOptions) {
                    if (dressSize.value == d) return dressSize;
                }

            });
            let defaultBodyType = currentProduct.bodyType.map(d => {
                for (const bodyType of OPTIONS.bodyTypeOptions) {
                    if (bodyType.value == d) return bodyType;
                }
            })
            setDefaultValues({ defaultDressSize, defaultBodyType });
            setLoading(false);
        }
    }, [currentProduct])
    ...
    const onSubmit = async () => {
        ...
        console.log(data);
        let success = await editProduct(product, currentProduct._id);
        setIsLoading(false);
        if (success) toast.success("Successfully updated product");
    };
    const onChange = (e) => {
        setdata({
            ...data,
        ...
        <div>
            <div className="row">
                ...
                <div className="col-md-4">
                    <div className="row">
                        <div className="col-8 pr-1">
                            <div className="form-group">
                                {showSelect ? <Select
                                    defaultValue={{ label: data.dressType, value: data.dressType }}
                ...
                <div className="col-md-6">
                    <div className="form-group">
                        <Select
                            defaultValue={defaultValues.defaultDressSize}
                            isMulti
                ...
                <div className="col-md-6">
                    <div className="form-group">
                        <Select
                            defaultValue={defaultValues.defaultBodyType}
                            isMulti
...
export default connect(mapStateToProps, { addProduct, loadDressTypes, setCurrentProduct, editProduct })(EditProductForm);
